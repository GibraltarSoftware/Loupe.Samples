...
namespace Gibraltar.TraceMonitorSamples
{
    ...
    /// </summary>
    /// <remarks><para>
    /// This example is compiled with direct reference to Loupe.  This allows access to the full
    /// features of the Loupe API, including custom metrics (not shown here).
    /// </para>
    /// <para>
    /// This example shows a typical WinForms Program.Main() with a few recommended calls to make sure the
    /// Loupe Agent can perform at its best.  It also shows an example of how your existing exception handling
    /// can use the Loupe API to invoke the Loupe Error Manager (or to just record the exception in the
    ...
    /// Loupe Agent without a direct reference.
    /// </para></remarks>
    internal static class Program
    {
        ...
        private static void Main()
        {
            ...
            // This next line is an example of how to programatically attach Loupe as a Trace Listener
            ...
            Trace.TraceInformation("Application starting."); // ...so the Loupe Agent will receive this message and initialize.

            // OR... You could initialize the Loupe Agent by just invoking the Agent directly. (You don't really need both.)
            ...
            // Also see the dynamic-binding example which uses only Loupe's automatic handling.
            ...
            Trace.Close(); // This also tells Loupe indirectly that we're exiting.
        ...
        private static void CurrentDomain_UnhandledException(object sender, UnhandledExceptionEventArgs e)
        {
            ...
            // events and records them automatically (unless you configure Loupe to ignore them by setting
            ...
            // full exception data (including stack trace).  Here's how you can do it (from anywhere) with the Loupe API:
            ...
            Log.Critical(exception, LogWriteMode.WaitForCommit, "Exception.Unhandled", "Unhandled exception event",
                ...
                "in your formatted message if you wish, in order to make them also available in the Loupe Live Viewer.\r\n" +
        ...
        private static void Application_ThreadException(object sender, ThreadExceptionEventArgs e)
        {
            ...
            // Although we've written our own handler, we can still get the Loupe Error Manager dialog with this explicit call.
            // This will report the error to the user via the Loupe Error Manager dialog, just as it would with the Agent's own handler.
